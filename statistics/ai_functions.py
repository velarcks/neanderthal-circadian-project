...
def merge_overlapping_regions(df):
    ...
    temp_df = pd.merge(df,temp_df,on='start').iloc[:,[0,1,-1]]
    temp_df.columns = temp_df.columns.str.replace(r'\_.*',r'',regex=True)
    return temp_df
...
def circadian_ai_enrichment(pred,SNPS,THRESHOLD,K):
